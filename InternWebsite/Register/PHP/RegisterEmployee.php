<?php
  if(isset($_POST['employerRegister'])) {
    $conn = mysqli_connect("localhost","root","","interncenter");
    ...
    if(mysqli_num_rows($checkDuplicate)> 0) {
        ...
        echo "<script>window.location.href = '../employer.html';</script>";
    }   
    else {
        $query = "INSERT into RegisterTable (email,username,password,userType) values ('$email', '$user', '$pass', 'Employer')";
        $data = mysqli_query($conn,$query);
        if($data) {
            echo "<script>alert('Registered as a Employer Successfully!');</script>";
